 ...
 * @link       http://phpmd.org
 */
class PHP_PMD_Rule_Padawan_EmptyTry
               ...
               PHP_PMD_Rule_IFunctionAware
{
    ...
    {
        foreach($node->findChildrenOfType('TryStatement') as $try) {
            foreach($try->getChildren() as $child) {
                if (null === $child->getImage()) {
                    $this->addViolation($try, array($node->getImage()));
                }
            }
        }
    }
