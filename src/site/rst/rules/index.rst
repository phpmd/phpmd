...
- `IfStatementAssignment`__: Having if conditions without any logic should be avoided. It usually tells us, that we are dealing with development code fragment, debug data, etc.
- `IfStatementWithoutLogic`__: Assignments in if clauses and the like are considered a code smell. Assignments in PHP return the right operand as their result. In many cases, this is an expected behavior, but can lead to many difficult to spot bugs, especially when the right operand could result in zero, null or an empty string.
...
__ design.html#ifstatementwithoutlogic
