...
{
    /**
     * @param array<string, list<string>|string> $args
     */
    ...
    {
        $args = $this->createInput(['paths' => ['source'], '--format' => 'xml', '--ruleset' => ['phpmd.xml'], '--baseline-file' => 'foobar.txt']);
    ...
    {
        $args = $this->createInput(['paths' => ['source'], '--format' => 'xml', '--ruleset' => [static::createResourceUriForTest('testA/phpmd.xml')]]);
        $finder = new BaselineFileFinder(new CommandLineOptions($args));
    ...
    public function testShouldThrowExceptionForNonExistingRuleSet(): void
    {
        ...
        $args = $this->createInput(['paths' => ['source'], '--format' => 'xml', '--ruleset' => [static::createResourceUriForTest('phpmd.xml')]]);
        $finder = new BaselineFileFinder(new CommandLineOptions($args));
        $finder->notNull()->find();
    }
    ...
    {
        $args = $this->createInput(['paths' => ['source'], '--format' => 'xml', '--ruleset' => [static::createResourceUriForTest('testB/phpmd.xml')]]);
        $finder = new BaselineFileFinder(new CommandLineOptions($args));
